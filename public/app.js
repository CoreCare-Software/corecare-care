...
const CORECARE_FALLBACK_VERSION = '1.19.3';
...
async function queueVisitEvent(type,code){const event={eventId:crypto.randomUUID(),type,code:code.trim(),deviceTime:new Date().toISOString(),source:navigator.onLine?'online':'offline'};const q=visitQueue();q.push(event);saveVisitQueue(q);if(!navigator.onLine){renderSyncStatus();return {offline:true};}const response=await api('/api/visits/sync',{method:'POST',body:JSON.stringify({events:[event]})});const result=(response.results||[])[0]||{};if(!result.ok){saveVisitQueue(visitQueue().filter(x=>x.eventId!==event.eventId));renderSyncStatus();throw new Error(result.error||'CoreCare could not record the visit event.');}saveVisitQueue(visitQueue().filter(x=>x.eventId!==event.eventId));await loadVisitsBoardNoSync();return result;}
...
$('#visit-clock-form')?.addEventListener('submit',async e=>{e.preventDefault();const form=e.currentTarget,data=Object.fromEntries(new FormData(form)),error=$('#visit-clock-error');if(error){error.hidden=true;error.textContent='';}try{const result=await queueVisitEvent(data.type,data.code);$('#visit-clock-dialog')?.close();form.reset();showSuccessToast?.(result.offline?'Visit event saved offline.':data.type==='clock_in'?'Clocked in successfully.':'Clocked out successfully.');}catch(ex){if(error){error.textContent=ex.message;error.hidden=false;}else showToastError?.(ex);}});
...
async function openVisitCareRecord(visitId){
 const data=await api(`/api/visits/${encodeURIComponent(visitId)}/care-record`),f=$('#visit-record-form'),v=data.visit,r=data.record||{},locked=Boolean(data.locked);f.reset();f.visitId.value=visitId;f.dataset.locked=locked?'true':'false';f.dataset.canAmend=data.canAmend?'true':'false';$('#visit-record-title').textContent=locked?'Completed care record':'Record visit care';$('#visit-record-meta').textContent=`${v.client_name||'Client'} · ${opFmt(v.scheduled_start)} · ${v.staff_name||'Unallocated'}${locked?' · Read-only':''}`;
 ...
 const submit=f.querySelector('button[type="submit"]'),amend=$('#visit-record-amend');[...f.elements].forEach(el=>{if(el.name!=='visitId'&&el!==submit)el.disabled=locked;});if(submit){submit.hidden=locked;submit.textContent='Save care record';}if(amend){amend.hidden=!(locked&&data.canAmend);amend.disabled=false;}$('#visit-record-lock-note').hidden=!locked;$('#visit-record-error').hidden=true;$('#visit-record-dialog')?.showModal();
}
$('#visit-record-form')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,err=$('#visit-record-error');err.hidden=true;const fd=new FormData(f),tasks=[...f.querySelectorAll('input[name="task"]:checked')].map(x=>({key:x.value,label:x.dataset.label,status:'completed'}));const medication=[{name:fd.get('medicationName'),outcome:fd.get('medicationOutcome'),reason:fd.get('medicationReason')}];const payload=Object.fromEntries(fd);payload.tasks=tasks;payload.medication=medication;payload.followUpRequired=f.followUpRequired.checked;payload.incidentRequired=f.incidentRequired.checked;if(f.dataset.amendmentReason)payload.amendmentReason=f.dataset.amendmentReason;try{await api(`/api/visits/${encodeURIComponent(fd.get('visitId'))}/care-record`,{method:'POST',body:JSON.stringify(payload)});delete f.dataset.amendmentReason;$('#visit-record-dialog')?.close();await loadVisitsBoardNoSync();showSuccessToast?.(payload.amendmentReason?'Care record amended with an audit entry.':'Care record saved and locked. Clock out when the visit is finished.');}catch(ex){err.textContent=ex.message;err.hidden=false;}});
$('#visit-record-amend')?.addEventListener('click',()=>{const f=$('#visit-record-form'),reason=prompt('Enter the reason for amending this completed care record. This will be recorded in the audit trail.');if(!reason?.trim())return;f.dataset.amendmentReason=reason.trim();[...f.elements].forEach(el=>{if(el.name!=='visitId')el.disabled=false;});const submit=f.querySelector('button[type="submit"]');if(submit){submit.hidden=false;submit.textContent='Save audited amendment';}$('#visit-record-amend').hidden=true;$('#visit-record-lock-note').hidden=true;});

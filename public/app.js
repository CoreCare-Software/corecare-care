...
document.addEventListener('change',event=>{const toggle=event.target.closest?.('[data-domain-enabled]');if(!toggle)return;const card=toggle.closest('[data-care-domain]');if(!card)return;const badge=card.querySelector('.care-domain-heading b');card.classList.toggle('enabled',toggle.checked);card.classList.toggle('domain-disabled',!toggle.checked);if(badge)badge.textContent=toggle.checked?'Included':'Not included';updateCarePlanProgress();const dialog=$('#care-plan-dialog');const form=$('#care-plan-form');if(dialog)dialog.style.display='';if(form)form.style.display='';});
document.addEventListener('click',event=>{const heading=event.target.closest?.('[data-domain-expand]');if(heading){const card=heading.closest('[data-care-domain]'),fields=card.querySelector('.care-domain-fields');if(card.querySelector('[data-domain-enabled]').checked)fields.classList.toggle('manually-collapsed');}const jump=event.target.closest?.('[data-plan-jump]');if(jump){event.preventDefault();$('#'+jump.dataset.planJump)?.scrollIntoView({behavior:'smooth',block:'start'});}});

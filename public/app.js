...
const CORECARE_FALLBACK_VERSION = '2.0.0';
...
function updateRotaTeamState(){const required=Math.max(1,Math.min(4,Number($('#rota-team-required')?.value)||1)),selected=document.querySelectorAll('#rota-team-list [data-rota-team-staff]:checked').length,state=$('#rota-team-state');if(!state)return;state.textContent=selected===required?'Team ready':selected?`${selected} of ${required} allocated`:`0 of ${required} allocated`;state.className=`badge ${selected===required?'success':selected?'warning':'danger'}`;}
async function loadRotaTeamEditor(id){rotaTeamEditorVisitId=id;rotaTeamEditorData=null;const list=$('#rota-team-list'),state=$('#rota-team-state');if(list)list.innerHTML='<p class="muted">Checking availability, training, competency and clashes…</p>';if(state){state.textContent='Checking';state.className='badge neutral';}try{const data=await api(`/api/rota/${encodeURIComponent(id)}/candidates`);if(rotaTeamEditorVisitId!==id||$('#rota-edit-form')?.elements.id.value!==id)return;rotaTeamEditorData=data;const currentIds=new Set((data.current||[]).map(row=>row.staff_id)),required=Math.max(1,Math.min(4,Number(data.visit?.carers_required)||1));if($('#rota-team-required'))$('#rota-team-required').value=String(required);if(list)list.innerHTML=(data.candidates||[]).map(candidate=>{const person=candidate.staff||{},selected=currentIds.has(person.id),blocked=!candidate.allowed,detail=blocked?(candidate.blockers||[]).map(item=>item.message).join(' · '):(candidate.warnings||[]).map(item=>item.message).join(' · ')||'Available and allocation checks passed';return `<label class="check-row rota-team-option ${blocked?'is-blocked':''}"><input type="checkbox" data-rota-team-staff value="${escapeHtml(person.id)}" ${selected?'checked':''} ${blocked&&!selected?'disabled':''}><span><strong>${escapeHtml(person.display_name||[person.first_name,person.last_name].filter(Boolean).join(' '))}</strong><small>${escapeHtml(person.job_title||'Care worker')} · ${escapeHtml(detail)}</small></span></label>`;}).join('')||'<p class="muted">No active care workers are available.</p>';list?.querySelectorAll('[data-rota-team-staff]').forEach(input=>input.addEventListener('change',updateRotaTeamState));updateRotaTeamState();}catch(error){if(list)list.innerHTML=`<p class="form-error">${escapeHtml(error.message)}</p>`;if(state){state.textContent='Could not check';state.className='badge danger';}}}
async function saveRotaTeamEditor(id,leadId){if(rotaTeamEditorVisitId!==id||!rotaTeamEditorData)throw new Error('Wait for the safe care-team checks to finish before saving.');const required=Math.max(1,Math.min(4,Number($('#rota-team-required')?.value)||1)),selected=[...document.querySelectorAll('#rota-team-list [data-rota-team-staff]:checked')].map(input=>input.value);if(leadId&&!selected.includes(leadId))selected.unshift(leadId);const ordered=[...(leadId&&selected.includes(leadId)?[leadId]:[]),...selected.filter(value=>value!==leadId)];if(ordered.length>required)throw new Error(`This visit requires ${required} care worker${required===1?'':'s'}. Remove ${ordered.length-required} selection${ordered.length-required===1?'':'s'} before saving.`);return api(`/api/rota/${encodeURIComponent(id)}/allocations`,{method:'PUT',body:JSON.stringify({carersRequired:required,assignments:ordered.map((staffId,index)=>({staffId,role:index===0?'lead':'support'}))})});}
const openRotaEditBase=openRotaEdit,patchRotaVisitBase=patchRotaVisit;
openRotaEdit=id=>{openRotaEditBase(id);void loadRotaTeamEditor(id);};
patchRotaVisit=async(id,payload,options)=>{const editor=$('#rota-edit-dialog'),editing=Boolean(editor?.open&&$('#rota-edit-form')?.elements.id.value===id);const result=await patchRotaVisitBase(id,payload,options);if(editing){await saveRotaTeamEditor(id,payload.staffId||'');await loadRotaBoard();await loadVisitsBoardNoSync();}return result;};
...
$('#rota-team-required')?.addEventListener('change',updateRotaTeamState);$('#rota-edit-staff')?.addEventListener('change',event=>{const chosen=event.currentTarget.value,input=chosen?document.querySelector(`#rota-team-list [data-rota-team-staff="${CSS.escape(chosen)}"]`):null;if(input&&!input.disabled)input.checked=true;updateRotaTeamState();});

...
function renderRotaVisualBoard(){const day=selectedRotaDay(),rows=rotaFilteredRows().filter(v=>v.scheduled_start.slice(0,10)===day),unallocated=rows.filter(v=>!v.staff_id),tray=$('#rota-unallocated-tray');if(tray){tray.innerHTML=unallocated.map(v=>`<article class="unallocated-card" draggable="true" data-rota-drag="${v.id}"><strong>${escapeHtml(v.client_name||'Client')}</strong><span>${new Date(v.scheduled_start).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})} · ${rotaVisitDuration(v)} mins</span><small>${escapeHtml(v.visit_type||'Care visit')}</small></article>`).join('')||'<span class="rota-drop-message">No unallocated visits for this day.</span>';}
 const scheduler=$('#rota-scheduler');if(!scheduler)return;const hours=Array.from({length:ROTA_END_HOUR-ROTA_START_HOUR},(_,i)=>ROTA_START_HOUR+i);scheduler.innerHTML=`<div class="scheduler-header"><div class="scheduler-corner">Carer</div>${hours.map(h=>`<div class="scheduler-hour">${String(h).padStart(2,'0')}:00</div>`).join('')}</div>`+(rotaData.staff||[]).map(st=>{const visits=rows.filter(v=>v.staff_id===st.id);return `<div class="scheduler-row"><div class="scheduler-staff"><strong>${escapeHtml(st.preferred_name||st.first_name)} ${escapeHtml(st.last_name)}</strong><small>${escapeHtml(st.job_title||'Carer')}</small></div><div class="scheduler-lane" data-staff-id="${st.id}">${visits.map(renderSchedulerVisit).join('')}</div></div>`}).join('')||'<div class="rota-drop-message">No active carers found.</div>';

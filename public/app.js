...
const CORECARE_FALLBACK_VERSION = '1.15.4';
...
function rotaDayRows(day=selectedRotaDay()){return (rotaData.visits||[]).filter(v=>String(v.scheduled_start).slice(0,10)===day&&rotaStatus(v)!=='cancelled');}
...
function calculateRotaIntelligence(day=selectedRotaDay()){
 ...
 return {rows,clashes,travel,availability,overtimeMinutes,continuity,score,warnings};
}
function renderRotaIntelligence(){const a=calculateRotaIntelligence();setVisitText('#rota-health-score',`${a.score}%`);setVisitText('#rota-clashes',a.clashes);setVisitText('#rota-travel-risks',a.travel);setVisitText('#rota-availability-risks',a.availability);setVisitText('#rota-continuity',`${a.continuity}%`);setVisitText('#rota-overtime',`${Math.round(a.overtimeMinutes/6)/10}h`);setVisitText('#rota-health-label',a.score>=90?'Healthy rota':a.score>=70?'Needs attention':'Action required');const el=$('#rota-intelligence-warnings');if(el)el.innerHTML=a.warnings.slice(0,8).map(w=>`<div class="planner-warning ${w.tone}"><span>${w.tone==='danger'?'⚠':'●'}</span><p>${escapeHtml(w.text)}</p></div>`).join('')||'<div class="planner-warning success"><span>✓</span><p>No immediate planning risks detected for the selected day.</p></div>';}
function candidateScore(visit,staffId){const dayRows=rotaDayRows().filter(v=>v.staff_id===staffId&&v.id!==visit.id).sort((a,b)=>new Date(a.scheduled_start)-new Date(b.scheduled_start)),start=new Date(visit.scheduled_start),end=new Date(visit.scheduled_end),overlap=dayRows.some(v=>start<new Date(v.scheduled_end)&&end>new Date(v.scheduled_start));if(overlap||!withinWorkingPattern(staffId,start,end))return -9999;let score=50;const preferred=(rotaData.preferredAssignments||[]).some(a=>a.client_id===visit.client_id&&a.staff_id===staffId);const familiar=(rotaData.visits||[]).filter(v=>v.client_id===visit.client_id&&v.staff_id===staffId).length;score+=preferred?35:0;score+=Math.min(20,familiar*4);const minutes=dayRows.reduce((n,v)=>n+rotaVisitDuration(v),0);score-=minutes/60*2;const prev=[...dayRows].reverse().find(v=>new Date(v.scheduled_end)<=start),next=dayRows.find(v=>new Date(v.scheduled_start)>=end);if(prev)score-=Math.max(0,30-(start-new Date(prev.scheduled_end))/60000)/3;if(next)score-=Math.max(0,30-(new Date(next.scheduled_start)-end)/60000)/3;return score;}
function buildOptimiseSuggestions(){const intelligence=calculateRotaIntelligence(),targets=intelligence.rows.filter(v=>!v.staff_id||v.travel_conflict||!withinWorkingPattern(v.staff_id,v.scheduled_start,v.scheduled_end));const suggestions=[];targets.forEach(v=>{const ranked=(rotaData.staff||[]).map(st=>({staff:st,score:candidateScore(v,st.id)})).filter(x=>x.score>-100).sort((a,b)=>b.score-a.score);const best=ranked[0];if(best&&best.staff.id!==v.staff_id)suggestions.push({visit:v,staffId:best.staff.id,staffName:`${best.staff.preferred_name||best.staff.first_name} ${best.staff.last_name}`,score:Math.round(best.score),reason:!v.staff_id?'Allocate unassigned visit':v.travel_conflict?'Reduce travel/clash risk':'Move inside availability'});});return suggestions;}
...
function renderSchedulerVisit(v){const d=new Date(v.scheduled_start),mins=(d.getHours()+d.getMinutes()/60-ROTA_START_HOUR)*ROTA_HOUR_WIDTH,duration=rotaVisitDuration(v),width=Math.max(42,duration/60*ROTA_HOUR_WIDTH),travel=Number(v.travel_before_minutes||0),travelWidth=travel/60*ROTA_HOUR_WIDTH,travelLeft=Math.max(0,mins-travelWidth);const travelBlock=travel>0?`<span class="scheduler-travel ${v.travel_conflict?'conflict':''}" style="left:${travelLeft}px;width:${Math.max(18,travelWidth)}px" title="${travel} minutes travel${v.travel_before_miles?` · ${Number(v.travel_before_miles).toFixed(1)} miles`:''}"><b>🚗 ${travel}m</b></span>`:'';const locked=Number(v.planner_locked)===1;return `${travelBlock}<article class="scheduler-visit ${escapeHtml(rotaStatus(v))} ${rotaVisitTypeClass(v)} ${v.travel_conflict?'travel-risk':''} ${locked?'is-locked':''} ${v.planner_notes?'has-notes':''}" draggable="${locked||(v.protected_time_rule&&v.protected_time_rule!=='flexible')?'false':'true'}" data-rota-drag="${v.id}" data-rota-open="${v.id}" style="left:${Math.max(0,mins)}px;width:${width}px" title="${escapeHtml(v.client_name||'Client')} – ${duration} minutes${travel?` · ${travel} minutes travel before`:''}${v.planner_notes?` · Note: ${escapeHtml(v.planner_notes)}`:''}"><span class="scheduler-visit-time">${d.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}</span><strong>${escapeHtml(v.client_name||'Client')}</strong><span>${escapeHtml(v.visit_type||'Care visit')} · ${duration} min</span>${v.travel_conflict?'<em>⚠ Travel shortfall</em>':''}${(v.template_id||v.recurrence_group_id)?'<i class="recurrence-badge" title="Recurring visit">🔄</i>':''}${(v.protected_time_rule&&v.protected_time_rule!=='flexible')?`<i class="recurrence-badge protected-time-badge" title="${escapeHtml(v.protected_time_reason||'Protected client visit time')}">🔐</i>`:''}${locked||(v.protected_time_rule&&v.protected_time_rule!=='flexible')?'':`<i class="scheduler-resize" data-rota-resize="${v.id}"></i>`}</article>`;}
...
function setTemplateTab(name){document.querySelectorAll('[data-template-tab]').forEach(x=>x.classList.toggle('active',x.dataset.templateTab===name));document.querySelectorAll('.template-panel').forEach(x=>x.classList.toggle('active',x.id===`template-${name}`));}
document.addEventListener('click',e=>{const b=e.target.closest?.('[data-template-tab]');if(!b)return;e.preventDefault();setTemplateTab(b.dataset.templateTab);});
function openTemplateVisitEditor(id){const item=(rotaTemplates.visitTemplates||[]).find(x=>x.id===id);if(!item)return;const d=$('#template-visit-dialog'),f=$('#template-visit-form');if(!d||!f)return;f.reset();const values={id:item.id,clientId:item.client_id,name:item.name||'Recurring care visit',dayOfWeek:item.day_of_week,preferredTime:item.preferred_time,durationMinutes:item.duration_minutes,windowMinutes:item.window_minutes,visitType:item.visit_type,carersRequired:item.carers_required,preferredStaffId:item.preferred_staff_id||'',backupStaffId:item.backup_staff_id||'',effectiveFrom:item.effective_from||'',effectiveTo:item.effective_to||'',notes:item.notes||''};Object.entries(values).forEach(([k,v])=>{if(f.elements[k])f.elements[k].value=v??''});d.showModal();}
document.addEventListener('dblclick',e=>{const card=e.target.closest?.('[data-template-edit="visit"]');if(card&&!e.target.closest('[data-template-delete]'))openTemplateVisitEditor(card.dataset.id);});

...
const WORKSPACE_CONFIG = {
  ...
  coordinator: {
    ...
    roles: ['office_staff'],
    pages: ['dashboard','clients','staff','family','care','medication','visits','rota','tasks','incidents','reports','support']
  },
  senior: {
    label: 'Senior carer workspace',
    roles: ['senior_carer'],
    pages: ['dashboard','staff','clients','care','medication','visits','rota','tasks','incidents','reports','support']
  },
  carer: {
...
const WORKSPACE_NAVIGATION = {
  ...
  senior: [
    ...
    ['Team responsibilities', [['rota','▦','Team rota'],['tasks','✓','Tasks'],['incidents','!','Incidents'],['reports','▥','Reports'],['support','?','Support']]]
  ],
  carer: [
...
async function saveFamilyAccountSettings(e){e.preventDefault();const form=e.currentTarget,err=$('#family-account-settings-error'),button=form.querySelector('[type="submit"]'),id=form.elements.accountId.value,payload={displayName:form.elements.displayName.value,status:form.elements.status.value,temporaryPassword:form.elements.temporaryPassword.value};err.hidden=true;button.disabled=true;try{const result=await api(`/api/family-access/accounts/${encodeURIComponent(id)}`,{method:'PUT',body:JSON.stringify(payload)});$('#family-account-settings-dialog').close();await loadFamilyManagement();if(result.emailDelivery)showEmailDeliveryResult(result.emailDelivery,'Family login updated.');}catch(error){err.textContent=error.message;err.hidden=false;}finally{button.disabled=false;}}
...
function openStaffDialog(id = '') {
  ...
  const managerSelect=$('#staff-line-manager');if(managerSelect)managerSelect.innerHTML='<option value="">Not assigned</option>'+staff.filter(row=>row.id!==id&&row.status==='Active').map(row=>`<option value="${escapeHtml(row.id)}">${escapeHtml(staffName(row))} · ${escapeHtml(row.jobTitle)}</option>`).join('');
...
const WORKFORCE_FORM_CONFIG={
  ...
  absences:{title:'Attendance and return-to-work record',description:'Restricted workforce record for absence management, support and safe return to work.',fields:[['absenceType','Absence type','select',['sickness|Sickness','authorised|Authorised absence','unpaid|Unpaid leave','family|Family / dependant','bereavement|Bereavement','medical|Medical appointment','other|Other'],true],['status','Status','select',['planned|Planned','open|Open','closed|Closed','cancelled|Cancelled'],true],['startedAt','Started','datetime-local',null,true],['endedAt','Ended','datetime-local'],['reasonSummary','Minimum necessary reason','textarea',null,false,'wide'],['fitNoteRequired','Fit note required','checkbox'],['fitNoteReceived','Fit note received','checkbox'],['returnToWorkRequired','Return-to-work meeting required','checkbox'],['returnToWorkCompletedAt','Return-to-work completed','datetime-local'],['returnToWorkNotes','Return-to-work discussion and adjustments','textarea',null,false,'wide']]},
...
function workforceFieldMarkup(field,record){
  ...
  else {let display=value;if(type==='datetime-local'&&display)display=String(display).replace(' ','T').slice(0,16);control=`<input name="${name}" type="${type}" value="${escapeHtml(display)}" ${type==='number'?'min="1" max="5"':''} ${required?'required':''}>`;}
...
function renderStaffAttendance(){const rows=activeStaffRecord.records.absences||[];$('#staff-record-content').innerHTML=`<section class="panel staff-section-panel restricted-panel"><div class="subheading"><div><p class="eyebrow">Restricted workforce data</p><h3>Attendance, wellbeing and return to work</h3><p>Access and changes are audited. Record only information necessary for employment support.</p></div>${workforceAddButton('absences','Record absence')}</div><div class="staff-record-list">${rows.map(row=>`<article><div><strong>${escapeHtml(row.absence_type.replaceAll('_',' '))}</strong><span>${dateTimeLabel(row.started_at)} to ${row.ended_at?dateTimeLabel(row.ended_at):'ongoing'}</span>${row.return_to_work_required?`<p>Return-to-work: ${row.return_to_work_completed_at?`completed ${dateTimeLabel(row.return_to_work_completed_at)}`:'required'}</p>`:''}</div><div>${workforceBadge(row.status)}${workforceAction('absences',row.id)}</div></article>`).join('')||workforceEmpty('No attendance records','Absence and return-to-work records will appear here.')}</div></section>`}
...
async function submitWorkforceRecord(event){event.preventDefault();const form=event.currentTarget,section=form.elements.section.value,recordId=form.elements.recordId.value,error=form.querySelector('.form-error'),button=$('#workforce-record-submit');error.hidden=true;button.disabled=true;button.textContent='Saving…';try{const endpoint=`/api/staff/${encodeURIComponent(activeStaffRecordId)}/workforce/${section}${recordId?`/${encodeURIComponent(recordId)}`:''}`;await api(endpoint,{method:recordId?'PUT':'POST',body:JSON.stringify(workforceFormPayload(form,section))});$('#workforce-record-dialog').close();await openStaffRecord(activeStaffRecordId,activeStaffRecordTab);if(!activeStaffRecord.access.self)await loadStaff()}catch(exception){error.textContent=exception.message;error.hidden=false}finally{button.disabled=false;button.textContent='Save record'}}
...
function showEmailDeliveryResult(delivery, successMessage = 'Account created.') {
  ...
  if (delivery?.status === 'not_configured') return showSuccessToast(`${successMessage} Automatic email is not configured yet. Give the temporary password to the user securely.`, 'warning');
  if (delivery?.status === 'not_supported') return showSuccessToast(`${successMessage} This software did not confirm automatic email support. Give the temporary password to the user securely.`, 'warning');
  if (delivery?.status === 'failed') return showSuccessToast(`${successMessage} The automatic email could not be sent. Give the temporary password to the user securely and ask an administrator to check email delivery.`, 'warning');
...
function renderAccessGovernance(){
  ...
  if(reviews)reviews.innerHTML=(accessGovernanceData.users||[]).map(user=>`<article class="role-card"><div class="role-card-icon">${initialsFromName(user.displayName)}</div><div><strong>${escapeHtml(user.displayName)}</strong><p>${escapeHtml(roleLabel(user.accessLevel))} · ${escapeHtml(user.email)}</p><small>${user.review.reviewedAt?`Last reviewed ${formatDateTime(user.review.reviewedAt)}${user.review.reviewerName?' by '+escapeHtml(user.review.reviewerName):''}`:'No completed access review'}${user.review.nextReviewDate?' · next '+formatDate(user.review.nextReviewDate):''}</small></div><span class="badge ${accessReviewBadge(user.review.state)}">${accessReviewLabel(user.review.state)}</span>${hasAccess('security.users.manage')&&user.id!==currentUser?.id?`<button type="button" class="row-action" data-review-access="${escapeHtml(user.id)}">Review</button>`:''}</article>`).join('')||'<p class="muted">No user accounts are available for review.</p>';
...
function openAccessReview(userId){const user=(accessGovernanceData.users||[]).find(item=>item.id===userId),form=$('#access-review-form');if(!user||!form)return;form.reset();form.elements.userId.value=userId;const profile=(accessGovernanceData.profiles||[]).find(item=>item.key===user.accessLevel),next=new Date();next.setUTCDate(next.getUTCDate()+Number(profile?.reviewDays||90));form.elements.nextReviewDate.value=next.toISOString().slice(0,10);$('#access-review-title').textContent=`Review ${user.displayName}`;$('#access-review-context').textContent=`${roleLabel(user.accessLevel)} · ${user.email}`;$('#access-review-error').hidden=true;$('#access-review-dialog')?.showModal();}
$$('[data-close-access-review]').forEach(button=>button.onclick=()=>$('#access-review-dialog')?.close());
$('#access-review-form')?.addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,error=$('#access-review-error'),data=Object.fromEntries(new FormData(form));error.hidden=true;try{await api('/api/security/access-reviews',{method:'POST',body:JSON.stringify(data)});$('#access-review-dialog')?.close();await Promise.all([loadAccessGovernance(),loadSettings()]);}catch(problem){error.textContent=problem.message;error.hidden=false;}});
...
function renderMedication(){
  ...
  list.innerHTML=meds.length?meds.map(m=>{const low=m.stock_quantity!==null&&Number(m.stock_quantity)<=Number(m.low_stock_threshold??5);return `<article class="record-card medication-card ${low?'medication-low-stock':''}"><div class="record-card-heading"><div><span class="badge ${m.status==='active'?'active':'neutral'}">${escapeHtml(m.status)}</span><h3>${escapeHtml(m.name)} ${escapeHtml(m.strength||'')}</h3></div><div><span class="badge ${m.is_prn?'warning':'neutral'}">${m.is_prn?'PRN':'Regular'}</span>${m.controlled_drug?'<span class="badge danger">Controlled · witness required</span>':m.requires_witness?'<span class="badge warning">Witness required</span>':''}</div></div><p><strong>${escapeHtml(m.dose)}</strong> · ${escapeHtml(m.route||'Route not recorded')}</p><p class="muted">${escapeHtml((m.scheduledTimes||[]).join(', ')||m.frequency||'No schedule recorded')}</p><p>${escapeHtml(m.instructions||'')}</p><p><b>Authorisation:</b> ${escapeHtml(m.prescriber_name||'Prescriber not recorded')}${m.authorisation_reference?` · ${escapeHtml(m.authorisation_reference)}`:''}</p><p><b>Stock:</b> ${m.stock_quantity===null?'Not tracked':escapeHtml(String(m.stock_quantity))+' '+escapeHtml(m.stock_unit||'')}${low?' · Low stock':''}</p>${m.is_prn&&m.prn_protocol?`<div class="notice-banner small-notice"><div><strong>PRN protocol</strong><span>${escapeHtml(m.prn_protocol)}${m.min_interval_minutes?` · Minimum interval ${escapeHtml(m.min_interval_minutes)} minutes`:''}</span></div></div>`:''}<div class="record-actions">${canAdminister&&m.status==='active'?`<button class="primary-button compact" data-administer-medication="${escapeHtml(m.id)}">Record administration</button>`:''}${canStock?`<button class="secondary-button compact" data-stock-medication="${escapeHtml(m.id)}">Stock</button>`:''}${canWrite?`<button class="secondary-button compact" data-edit-medication="${escapeHtml(m.id)}">Edit</button>`:''}</div></article>`;}).join(''):'<p class="muted">No medications recorded for this client.</p>';

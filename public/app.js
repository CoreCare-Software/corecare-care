...
const CORECARE_FALLBACK_VERSION = '1.31.0';
...
async function loadFamilyManagement(){
  ...
  const [accessPayload,userPayload,clientPayload]=await Promise.all([api('/api/family-access'),api('/api/users'),api('/api/clients')]);
  familyManagementData={links:accessPayload.links||[],users:(userPayload.users||[]).filter(x=>x.accessLevel==='family'&&x.status==='active'),clients:clientPayload.clients||[]};
  const active=familyManagementData.links.filter(x=>x.status==='active'),linkedUsers=new Set(active.map(x=>x.userId)),unlinked=familyManagementData.users.filter(x=>!linkedUsers.has(x.id));
  root.innerHTML=`<div class="family-management-hero"><div><p class="eyebrow">Family access management</p><h2>Connect relatives securely</h2><p>Choose exactly which client and record types each family login can see.</p></div><div class="family-management-actions"><button type="button" class="secondary-button compact" data-page-link="settings">Create family login</button><button id="family-access-new" type="button" class="primary-button compact" ${!familyManagementData.users.length||!familyManagementData.clients.length?'disabled':''}>+ Grant access</button></div></div><section class="family-portal-metrics">${dashboardMetricCard('Active links',active.length,'family-to-client connections')}${dashboardMetricCard('Family accounts',familyManagementData.users.length,'active logins')}${dashboardMetricCard('Clients linked',new Set(active.map(x=>x.clientId)).size,'with family access')}${dashboardMetricCard('Awaiting link',unlinked.length,'family accounts')}</section><section class="family-management-grid"><article class="panel span-two"><div class="panel-heading"><div><p class="eyebrow">Access register</p><h2>Who can see what</h2></div></div><div class="family-access-list">${active.map(link=>`<article class="family-access-card"><div class="family-access-person"><span class="person-avatar">${escapeHtml(initialsFromName(link.displayName))}</span><div><strong>${escapeHtml(link.displayName)}</strong><span>${escapeHtml(link.email)} · ${escapeHtml(link.clientName)}</span></div></div><div class="family-access-chips">${familyPermissionLabels({profile:link.canViewProfile,visits:link.canViewVisits,careUpdates:link.canViewCareUpdates,documents:link.canViewDocuments,medication:link.canViewMedication}).map(x=>`<span>${escapeHtml(x)}</span>`).join('')}</div><div class="record-actions"><button class="row-action" data-family-edit="${escapeHtml(link.id)}">Edit</button><button class="row-action danger" data-family-revoke="${escapeHtml(link.id)}">Revoke</button></div></article>`).join('')||'<div class="empty-state"><strong>No family access granted</strong><span>Create a family login in Settings, then link it to a client here.</span></div>'}</div></article><aside class="panel"><div class="panel-heading"><div><p class="eyebrow">Not linked</p><h2>Family accounts</h2></div></div><div class="family-unlinked-list">${unlinked.map(u=>`<button type="button" data-family-user="${escapeHtml(u.id)}"><strong>${escapeHtml(u.displayName)}</strong><span>${escapeHtml(u.email)}</span><small>Grant client access</small></button>`).join('')||'<p class="padded muted">Every active family account has a client link.</p>'}</div></aside></section><dialog id="family-access-dialog" class="client-dialog"><form id="family-access-form"><div class="dialog-heading"><div><p class="eyebrow">Family portal</p><h2>Grant family access</h2></div><button type="button" class="icon-button" data-close-family-access>×</button></div><input type="hidden" name="accessId"><div class="form-grid"><label><span>Family account</span><select name="userId" required><option value="">Select family account</option>${familyManagementData.users.map(u=>`<option value="${escapeHtml(u.id)}">${escapeHtml(u.displayName)} · ${escapeHtml(u.email)}</option>`).join('')}</select></label><label><span>Client</span><select name="clientId" required><option value="">Select client</option>${familyManagementData.clients.map(c=>`<option value="${escapeHtml(c.id)}">${escapeHtml(clientDisplayName(c))}</option>`).join('')}</select></label></div><fieldset class="family-access-options"><legend>Information this login may see</legend><label><input type="checkbox" name="canViewProfile"> Profile summary</label><label><input type="checkbox" name="canViewVisits"> Published visits</label><label><input type="checkbox" name="canViewCareUpdates"> Completed care updates</label><label><input type="checkbox" name="canViewDocuments"> Shared documents</label><label><input type="checkbox" name="canViewMedication"> Active medication</label></fieldset><p id="family-access-error" class="form-error" hidden></p><div class="dialog-actions"><button type="button" class="secondary-button" data-close-family-access>Cancel</button><button type="submit" class="primary-button compact">Save access</button></div></form></dialog>`;
  bindPageLinks(root);$('#family-access-new')?.addEventListener('click',()=>openFamilyAccessDialog());$$('[data-family-edit]').forEach(b=>b.addEventListener('click',()=>openFamilyAccessDialog(b.dataset.familyEdit)));$$('[data-family-user]').forEach(b=>b.addEventListener('click',()=>openFamilyAccessDialog('',b.dataset.familyUser)));$$('[data-family-revoke]').forEach(b=>b.addEventListener('click',async()=>{if(!confirm('Revoke this family member’s access to the client record?'))return;await api(`/api/family-access/${encodeURIComponent(b.dataset.familyRevoke)}`,{method:'DELETE'});await loadFamilyManagement();}));$$('[data-close-family-access]').forEach(b=>b.addEventListener('click',()=>$('#family-access-dialog')?.close()));$('#family-access-form')?.addEventListener('submit',saveFamilyAccessForm);
}
...
function renderIncidentsWorkspace(){const list=$('#incidents-workspace-list');if(!list)return;const incidents=operationsData.incidents||[],open=incidents.filter(i=>i.status!=='closed');setText('#incidents-open-count',open.length);setText('#incidents-critical-count',open.filter(i=>i.severity==='critical').length);setText('#incidents-high-count',open.filter(i=>i.severity==='high').length);setText('#incidents-closed-count',incidents.filter(i=>i.status==='closed').length);
  const search=($('#incidents-search')?.value||'').trim().toLowerCase(),status=$('#incidents-status-filter')?.value||'open',severity=$('#incidents-severity-filter')?.value||'all';const filtered=incidents.filter(i=>{const haystack=[i.title,i.description,i.client_name,i.category,i.manager_review].join(' ').toLowerCase();return(!search||haystack.includes(search))&&(status==='all'||status==='open'&&i.status!=='closed'||i.status===status)&&(severity==='all'||i.severity===severity)});$('#incidents-result-summary').textContent=`Showing ${filtered.length} of ${incidents.length} incident${incidents.length===1?'':'s'}`;
  list.innerHTML=filtered.map(i=>`<article class="module-record"><span class="module-record-indicator ${escapeHtml(i.status==='closed'?'closed':i.severity||'medium')}"></span><div class="module-record-main"><div class="module-record-heading"><h3>${escapeHtml(i.title)}</h3><span class="badge ${incidentBadgeClass(i.severity,i.status)}">${escapeHtml(i.severity||'medium')}</span><span class="badge ${i.status==='closed'?'success':'neutral'}">${escapeHtml(i.status||'open')}</span></div><p>${escapeHtml(i.description||'No description recorded.')}</p><div class="module-record-meta"><span>Client: ${escapeHtml(i.client_name||'No client linked')}</span><span>Category: ${escapeHtml(i.category||'General')}</span><span>Occurred: ${opFmt(i.occurred_at||i.created_at)}</span>${i.reviewed_at?`<span>Reviewed: ${opFmt(i.reviewed_at)}</span>`:''}</div>${i.manager_review?`<p class="incident-review-outcome"><strong>Review outcome:</strong> ${escapeHtml(i.manager_review)}</p>`:''}</div><div class="module-record-actions">${canWriteOperations('incidents.manage')&&i.status!=='closed'?`<button data-op-review="${escapeHtml(i.id)}">Review and close</button>`:i.status!=='closed'?'<span class="badge neutral">Awaiting review</span>':'<span class="badge success">Review complete</span>'}</div></article>`).join('')||'<div class="module-empty-state"><strong>No matching incidents</strong><span>Change the filters or record a new incident.</span></div>';
  const review=$('#incidents-review-summary');review.innerHTML=open.slice().sort((a,b)=>({critical:0,high:1,medium:2,low:3}[a.severity]??4)-({critical:0,high:1,medium:2,low:3}[b.severity]??4)).slice(0,8).map(i=>`<article class="module-summary-item"><div><strong>${escapeHtml(i.title)}</strong><small>${escapeHtml(i.severity||'medium')} · ${escapeHtml(i.client_name||i.category||'General')} · ${opFmt(i.occurred_at||i.created_at)}</small></div><b>!</b></article>`).join('')||'<div class="module-empty-state"><strong>Review queue clear</strong><span>There are no open incidents.</span></div>';
}
...
$('#operations-new-task')?.addEventListener('click',()=>canWriteOperations('tasks.manage')?$('#operations-task-dialog')?.showModal():denyPage());$('#tasks-new')?.addEventListener('click',()=>canWriteOperations('tasks.manage')?$('#operations-task-dialog')?.showModal():denyPage());$('#operations-record-incident')?.addEventListener('click',()=>canWriteOperations('incidents.manage')?$('#operations-incident-dialog')?.showModal():denyPage());$('#incidents-new')?.addEventListener('click',()=>canWriteOperations('incidents.manage')?$('#operations-incident-dialog')?.showModal():denyPage());$('#operations-add-handover')?.addEventListener('click',()=>canWriteOperations('operations.manage')?$('#operations-handover-dialog')?.showModal():denyPage());$$('[data-close-dialog]').forEach(b=>b.addEventListener('click',()=>document.getElementById(b.dataset.closeDialog)?.close()));

...
async function moveRotaVisit(id,staffId,startDate,duration){const v=(rotaData.visits||[]).find(x=>x.id===id);if(!v)return;const start=new Date(startDate),end=new Date(start.getTime()+(duration||rotaVisitDuration(v))*60000);try{await api(`/api/rota/${id}`,{method:'PATCH',body:JSON.stringify({clientId:v.client_id,staffId:staffId||'',visitType:v.visit_type,scheduledStart:start.toISOString(),scheduledEnd:end.toISOString(),scope:'single',reason:'Planner drag-and-drop adjustment'})});await loadRotaBoard();await loadVisitsBoardNoSync();}catch(e){alert(e.message);}}

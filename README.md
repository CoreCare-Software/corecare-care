...
CoreCare Care is a multi-tenant care-management web application deployed as a Cloudflare Worker with static browser assets and a Cloudflare D1 database. The current package version is `2.0.3`.

Release 2.0.3 turns Organisation settings > Modules into a complete live module catalogue. Optional areas can be enabled or hidden organisation-wide, changes apply immediately, disabled APIs are protected on the server and new organisations receive the full catalogue automatically. It builds on the support-safe onboarding and branch lifecycle controls delivered in 2.0.2.

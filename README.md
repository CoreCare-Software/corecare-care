...
CoreCare Care is a multi-tenant care-management web application deployed as a Cloudflare Worker with static browser assets and a Cloudflare D1 database. The current package version is `2.0.4`.

Release 2.0.4 makes each printed client QR code permanent and client-bound until an authorised manager deliberately regenerates it. It retains revoked-code history, supports repeated safe regeneration and strengthens the organisation/client database boundary. It builds on the complete organisation module controls delivered in 2.0.3.

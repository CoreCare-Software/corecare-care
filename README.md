...
CoreCare Care is a multi-tenant care-management web application deployed as a Cloudflare Worker with static browser assets and a Cloudflare D1 database. The current package version is `2.2.0`.

Release 2.2.0 introduces the multi-branch rota command centre: fast organisation-wide and branch-focused views, local-first explainable optimisation, governed cross-branch cover, visible branch ownership, and acknowledgement-required notifications when a published rota changes. It builds on the opt-in, private-broker AI rewriting delivered in 2.1.0.
...
Migration `0062_branch_aware_rota.sql` adds branch-aware care-team assignments, controlled cross-branch cover, published-rota snapshots and care-worker change acknowledgements used by release 2.2.0.

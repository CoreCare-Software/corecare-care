...
CoreCare Care is a multi-tenant care-management web application deployed as a Cloudflare Worker with static browser assets and a Cloudflare D1 database. The current package version is `2.0.2`.

Release 2.0.2 restores production-safe account onboarding in the Cloudflare runtime, binds Platform support sessions to the customer’s active branch, adds durable privacy-safe API error recording and introduces controlled branch archive and restore. It builds on the holiday-safe rota protections delivered in 2.0.1.

...
CoreCare Care is a multi-tenant care-management web application deployed as a Cloudflare Worker with static browser assets and a Cloudflare D1 database. The current package version is `2.0.1`.

Release 2.0.1 adds approved annual leave to the staff record and protects every rota allocation path from holiday or active-absence conflicts. Saving leave safely unallocates overlapping future visits, returns published visits to draft, creates persistent manager exceptions and requires cover before republishing. It builds on the multi-carer rota, care assurance, native Authenticator MFA and commercial workflows delivered in 2.0.0.

...
function showRotaContextMenu(e,id){e.preventDefault();selectRotaVisit(id,e.ctrlKey||e.metaKey);rotaContextVisitId=id;const menu=$('#rota-context-menu');if(!menu)return;menu.hidden=false;menu.style.left=`${Math.min(e.clientX,window.innerWidth-250)}px`;menu.style.top=`${Math.min(e.clientY,window.innerHeight-430)}px`;const v=(rotaData.visits||[]).find(x=>x.id===id),lock=menu.querySelector('[data-rota-action="lock"]'),isRecurring=Boolean(v?.template_id||v?.recurrence_group_id);if(lock)lock.textContent=Number(v?.planner_locked)?'Unlock visit':'Lock visit';const recurring=menu.querySelector('[data-rota-action="recurring"]'),pause=menu.querySelector('[data-rota-action="pause-recurrence"]'),resume=menu.querySelector('[data-rota-action="resume-recurrence"]'),stop=menu.querySelector('[data-rota-action="stop-recurrence"]');if(recurring){recurring.hidden=false;recurring.textContent=isRecurring?'🔄 Change recurrence':'🔄 Make recurring';}if(pause)pause.hidden=!isRecurring||v?.recurrence_status==='paused';if(resume)resume.hidden=!isRecurring||v?.recurrence_status!=='paused';if(stop)stop.hidden=!isRecurring;}

function openRotaRecurrence(id){hideRotaContextMenu();const v=(rotaData.visits||[]).find(x=>x.id===id);if(!v)return;const f=$('#rota-recurrence-form');if(!f)return alert('The recurring-visit editor could not be opened. Please refresh CoreCare and try again.');f.reset();const isExisting=Boolean(v.template_id||v.recurrence_group_id);f.elements.visitId.value=id;f.elements.action.value=isExisting?'update':'create';f.elements.keepCarer.checked=v.recurrence_keep_carer==null?true:Boolean(Number(v.recurrence_keep_carer));const start=new Date(v.scheduled_start),day=((start.getDay()+6)%7)+1,existingDays=String(v.recurrence_days||'').split(',').map(Number).filter(Boolean);f.elements.intervalWeeks.value=String(Number(v.recurrence_interval_weeks)||1);f.elements.effectiveFrom.value=v.recurrence_effective_from||start.toISOString().slice(0,10);f.querySelectorAll('input[name="days"]').forEach(x=>x.checked=(existingDays.length?existingDays:[day]).includes(Number(x.value)));if(v.recurrence_effective_to){f.elements.endMode.value='date';f.elements.effectiveTo.value=v.recurrence_effective_to;}else if(Number(v.recurrence_end_after_occurrences)>0){f.elements.endMode.value='count';f.elements.endAfterOccurrences.value=String(v.recurrence_end_after_occurrences);}else{f.elements.endMode.value='never';}const title=$('#rota-recurrence-title'),summary=$('#rota-recurrence-summary'),submit=f.querySelector('[data-recurrence-submit],button[type="submit"],.dialog-actions .primary-button'),err=$('#rota-recurrence-error');if(title)title.textContent=isExisting?'Change recurring visit':'Make this visit recurring';if(summary)summary.textContent=`${v.client_name||'Client'} · ${v.visit_type||'Care visit'} · ${start.toLocaleDateString('en-GB',{weekday:'long'})} ${start.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})} · ${v.staff_name||'Unallocated'}`;if(submit)submit.textContent=isExisting?'Update recurrence':'Create recurring visit';if(err)err.hidden=true;updateRecurrenceEndFields();const dialog=$('#rota-recurrence-dialog');if(dialog?.showModal)dialog.showModal();}
...
function recurringSeriesClass(v){if(!v?.recurrence_group_id&&!v?.template_id)return '';const key=String(v.recurrence_group_id||v.template_id);let hash=0;for(const ch of key)hash=(hash*31+ch.charCodeAt(0))>>>0;return `recurrence-series-${hash%6}`;}
function chooseRecurringEditScope(v,actionLabel='change'){if(!v?.template_id&&!v?.recurrence_group_id)return 'single';const answer=prompt(`This is a recurring visit. Apply this ${actionLabel} to:

1 — This visit only
2 — This and future visits
3 — Entire recurring series

Enter 1, 2 or 3.`, '1');if(answer===null)return null;return answer.trim()==='2'?'future':answer.trim()==='3'?'series':'single';}
...
async function moveRotaVisit(id,staffId,startDate,duration,travelOverrideReason=''){const v=(rotaData.visits||[]).find(x=>x.id===id);if(!v)return;if(Number(v.planner_locked))return alert('This visit is locked. Open it and unlock it before moving.');const scope=chooseRecurringEditScope(v,duration?'duration change':'move');if(scope===null)return;const start=new Date(startDate),end=new Date(start.getTime()+(duration||rotaVisitDuration(v))*60000),payload={clientId:v.client_id,staffId:staffId||'',visitType:v.visit_type,scheduledStart:start.toISOString(),scheduledEnd:end.toISOString(),scope,reason:'Planner drag-and-drop adjustment',travelOverrideReason,plannerNotes:v.planner_notes||'',plannerLocked:false};try{await patchRotaVisit(id,payload);}catch(e){alert(e.message);}}
...
// CoreCare 1.13.2 — Recurring Visit Workflow Hotfix

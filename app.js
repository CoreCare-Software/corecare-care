...
let rotaView='board';
const ROTA_START_HOUR=6, ROTA_END_HOUR=20, ROTA_HOUR_WIDTH=120;
...
function rotaFilteredRows(){const status=$('#rota-status-filter')?.value||'all';let rows=rotaData.visits||[];if(status==='unallocated')rows=rows.filter(v=>!v.staff_id);else if(status!=='all')rows=rows.filter(v=>rotaStatus(v)===status);return rows;}
...
function renderRotaVisualBoard(){const day=selectedRotaDay(),rows=rotaFilteredRows().filter(v=>v.scheduled_start.slice(0,10)===day),unallocated=rows.filter(v=>!v.staff_id),tray=$('#rota-unallocated-tray');if(tray){tray.innerHTML=unallocated.map(v=>`<article class="unallocated-card" draggable="true" data-rota-drag="${v.id}"><div class="visit-card-top"><span class="visit-time">${new Date(v.scheduled_start).toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}</span><span class="visit-duration">${rotaVisitDuration(v)} min</span></div><strong>${escapeHtml(v.client_name||'Client')}</strong><small>${escapeHtml(v.visit_type||'Care visit')}</small><span class="visit-card-action">Drag to allocate →</span></article>`).join('')||'<div class="rota-empty-inline"><span class="empty-icon">✓</span><div><strong>Nothing waiting to be allocated</strong><small>New unassigned visits will appear here automatically.</small></div></div>';}
 const scheduler=$('#rota-scheduler');if(!scheduler)return;const hours=Array.from({length:ROTA_END_HOUR-ROTA_START_HOUR},(_,i)=>ROTA_START_HOUR+i),staff=rotaData.staff||[];if(!staff.length){scheduler.innerHTML=`<div class="scheduler-empty-state"><div class="scheduler-empty-icon">👥</div><strong>Add active care staff to begin planning</strong><span>The scheduling grid is created from active staff records. Add a staff member, then return here to allocate visits.</span><button type="button" class="primary-button compact" data-page-link="staff">Open staff</button></div>`;scheduler.querySelector('[data-page-link]')?.addEventListener('click',()=>navigateTo('staff'));return;}scheduler.innerHTML=`<div class="scheduler-header"><div class="scheduler-corner"><span>Care team</span><small>${new Date(day+'T12:00:00').toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long'})}</small></div>${hours.map(h=>`<div class="scheduler-hour"><strong>${String(h).padStart(2,'0')}:00</strong><span>${h<12?'Morning':h<17?'Afternoon':'Evening'}</span></div>`).join('')}</div>`+staff.map(st=>{const visits=rows.filter(v=>v.staff_id===st.id);return `<div class="scheduler-row"><div class="scheduler-staff"><span class="staff-avatar">${escapeHtml(((st.preferred_name||st.first_name||'?')[0]+(st.last_name||'')[0]).toUpperCase())}</span><div><strong>${escapeHtml(st.preferred_name||st.first_name)} ${escapeHtml(st.last_name)}</strong><small>${escapeHtml(st.job_title||'Care worker')} · ${visits.length} visit${visits.length===1?'':'s'}</small></div></div><div class="scheduler-lane" data-staff-id="${st.id}">${visits.map(renderSchedulerVisit).join('')}</div></div>`}).join('');
 ...
 wireRotaDragging();wireRotaResize();}
function renderSchedulerVisit(v){const d=new Date(v.scheduled_start),mins=(d.getHours()+d.getMinutes()/60-ROTA_START_HOUR)*ROTA_HOUR_WIDTH,duration=rotaVisitDuration(v),width=Math.max(42,duration/60*ROTA_HOUR_WIDTH);return `<article class="scheduler-visit ${escapeHtml(rotaStatus(v))}" draggable="true" data-rota-drag="${v.id}" style="left:${Math.max(0,mins)}px;width:${width}px" title="${escapeHtml(v.client_name||'Client')} – ${duration} minutes"><span class="scheduler-visit-time">${d.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'})}</span><strong>${escapeHtml(v.client_name||'Client')}</strong><span>${escapeHtml(v.visit_type||'Care visit')} · ${duration} min</span><i class="scheduler-resize" data-rota-resize="${v.id}"></i></article>`;}

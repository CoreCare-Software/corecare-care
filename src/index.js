...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/version") return json({ name: "CoreCare", version: VERSION, release: "CoreCare Enterprise 1.25.1 — Care-plan toggle stability" });
...
async function getOrganisationTicket(db,session,id){if(!supportRoleAllowed(session))return forbidden();const ticket=await organisationTicketOrNull(db,session,id);if(!ticket)return notFound('Ticket');const [m,a,h]=await Promise.all([db.prepare(`SELECT m.*,u.display_name author_name FROM platform_ticket_messages m LEFT JOIN users u ON u.id=m.author_user_id WHERE m.ticket_id=? AND m.message_type!='internal_note' ORDER BY m.created_at`).bind(id).all(),db.prepare(`SELECT id,file_name,mime_type,size_bytes,uploaded_at FROM platform_ticket_attachments WHERE ticket_id=? ORDER BY uploaded_at`).bind(id).all(),db.prepare(`SELECT h.*,u.display_name changed_name FROM platform_ticket_status_history h LEFT JOIN users u ON u.id=h.changed_by WHERE h.ticket_id=? ORDER BY h.changed_at`).bind(id).all()]);return json({ticket,messages:m.results||[],attachments:a.results||[],history:h.results||[]})}

...
const RELEASE = "CoreCare Care 1.31.0 — Live rotas and connected workspaces";
...
async function createBodyMapRecord(request,db,session){
  ...
  if(!await ensureOrganisationClient(db,session,clientId))return json({error:{code:'NOT_FOUND',message:'Client not found.'}},404); const id=crypto.randomUUID(),severity=clean(i.severity)||'medium',concernType=clean(i.concernType)||'Other';
  ...
  if(i.createIncident||['high','critical'].includes(severity))statements.push(db.prepare(`INSERT INTO operations_incidents(id,organisation_id,client_id,reported_by,category,severity,title,description,status,occurred_at) VALUES(?,?,?,?,?,?,?,?,?,?)`).bind(crypto.randomUUID(),session.organisation_id,clientId,session.user_id,'Body map',severity,`${concernType}: ${clean(i.bodyLocation)||'body-map concern'}`,`Body map record ${id}. ${description}${clean(i.actionTaken)?` Action taken: ${clean(i.actionTaken)}`:''}`,'open',new Date(clean(i.firstObservedAt)||Date.now()).toISOString()));
...
async function updateBodyMapRecord(request,db,session,id){
  ...
  if(i.createIncident||(['high','critical'].includes(severity)&&(!['high','critical'].includes(row.severity)||row.status==='resolved')))statements.push(db.prepare(`INSERT INTO operations_incidents(id,organisation_id,client_id,reported_by,category,severity,title,description,status,occurred_at) VALUES(?,?,?,?,?,?,?,?,?,?)`).bind(crypto.randomUUID(),session.organisation_id,row.client_id,session.user_id,'Body map',severity,`${row.concern_type}: ${row.body_location||'body-map concern'}`,`Body map record ${id}${row.status==='resolved'?' reopened':' escalated'}. ${note}`,'open',new Date().toISOString()));

...
async function requireSession(request, db) {
  ...
  const row = await db.prepare(`SELECT s.id AS session_id,s.expires_at,s.user_id,s.organisation_id,u.email,u.display_name,u.role,u.status,u.must_change_password,o.name AS organisation_name FROM sessions s JOIN users u ON u.id=s.user_id JOIN organisations o ON o.id=s.organisation_id WHERE s.token_hash=? AND s.expires_at>CURRENT_TIMESTAMP LIMIT 1`).bind(await sha256Base64(token)).first();
...
async function listClients(db, session, url) {
  ...
  const sql = `SELECT ${CLIENT_COLUMNS} FROM clients WHERE organisation_id=? ${includeArchived ? "" : "AND status<>'Archived'"} ORDER BY last_name COLLATE NOCASE,first_name COLLATE NOCASE`;
  const result = await db.prepare(sql).bind(session.organisation_id).all();
  return json({ clients: result.results.map(toClient) });
}

async function getClient(db, session, id) {
  const row = await db.prepare(`SELECT ${CLIENT_COLUMNS} FROM clients WHERE id=? AND organisation_id=? LIMIT 1`).bind(id, session.organisation_id).first();
...
async function createClient(request, db, session) {
  ...
  await db.batch([
    db.prepare(`INSERT INTO clients (id,organisation_id,${fields.names.join(",")}) VALUES (?, ?, ${fields.names.map(() => "?").join(",")})`).bind(id, session.organisation_id, ...fields.values),
...
async function listUsers(db, session) {
  ...
  const result = await db.prepare("SELECT id,email,display_name,role,status,must_change_password,last_login_at,created_at FROM users WHERE organisation_id=? ORDER BY display_name COLLATE NOCASE").bind(session.organisation_id).all();
...
async function createUser(request, db, session) {
  ...
  if (!email || !name || !allowedRoles().includes(role) || password.length < 12) return json({ error: { code: "VALIDATION_ERROR", message: "Enter a name, valid email, role and temporary password of at least 12 characters." } }, 400);
  const secured = await hashPassword(password);
  const id = crypto.randomUUID();
  try {
    await db.batch([
      db.prepare("INSERT INTO users (id,organisation_id,email,display_name,role,password_hash,password_salt,password_iterations,status,must_change_password) VALUES (?,?,?,?,?,?,?,?, 'active',1)").bind(id, session.organisation_id, email, name, role, secured.hash, secured.salt, PASSWORD_ITERATIONS),
      auditStatement(db, session.organisation_id, session.user_id, "user.created", "user", id, { email, role })
    ]);
  } catch (error) {
    if (String(error).includes("UNIQUE")) return json({ error: { code: "EMAIL_EXISTS", message: "A user with that email already exists." } }, 409);
    throw error;
  }
  return json({ user: { id, email, displayName: name, role, status: "active", mustChangePassword: true } }, 201);
}
...
async function updateUser(request, db, session, id) {
  ...
  if (!name || !allowedRoles().includes(role) || !["active", "disabled"].includes(status)) return json({ error: { code: "VALIDATION_ERROR", message: "Enter a name, valid role and status." } }, 400);
  const result = await db.prepare("UPDATE users SET display_name=?,role=?,status=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND organisation_id=?").bind(name, role, status, id, session.organisation_id).run();
  ...
  await audit(db, session.organisation_id, session.user_id, "user.updated", "user", id, { role, status });
  return json({ ok: true });
}
...
function publicUser(row) { return { id: row.user_id || row.id, organisationId: row.organisation_id, organisationName: row.organisation_name, email: row.email, displayName: row.display_name, role: row.role, mustChangePassword: Boolean(row.must_change_password) }; }
function toUser(row) { return { id: row.id, email: row.email, displayName: row.display_name, role: row.role, status: row.status, mustChangePassword: Boolean(row.must_change_password), lastLoginAt: row.last_login_at, createdAt: row.created_at }; }

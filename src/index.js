...
async function reportsWorkspace(db,session,url){
  ...
  return json({range,summary:{...summary,clients:{active:activeClients.length,highRisk:highRiskClients}},dailyVisits:[...dailyMap.values()].sort((a,b)=>a.date.localeCompare(b.date)),incidentCategories:[...categoryMap.values()].sort((a,b)=>b.total-a.total),recentIncidents:incidentRows.slice(0,100),recentVisits:visitRows.slice(-100).reverse(),finance,canExport,generatedAt:new Date().toISOString(),recordLimitReached:[visitRows,incidentRows,tasks.results||[]].some(rows=>rows.length>=10000)});
}
